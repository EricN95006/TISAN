...
double absoluteGaussian(double x, double q[]);
double doubleGaussian(double x, double q[]);
double exponential(double x, double q[]);
double absoluteExponential(double x, double q[]);
...
*/
   ...
   switch (ITYPE)
      ...
      case 1: // Gaussian
         ...
         break;
      case 2: // Absolute Gaussian
         qCount = 4;
         function = absoluteGaussian;
         zTaskMessage(3,"Fitting Absolute Gaussian with %d Parameters\n",qCount);
         break;
      ...
      case 4: // Exponential
         ...
         break;
      case 5: // Absolute Exponential
         qCount = 3;
         function = absoluteExponential;
         zTaskMessage(3,"Fitting Absolute Exponential with %d Parameters\n",qCount);
         break;
      case 6: // A Sin(w t + phi) + B
         qCount = 4;
...
** ITYPE = 2
**
** Absolute Gaussian of the form y = q[0]^2 exp((-(x - q[1])^2)*(q[2]^2)) + q[3]^2
**
** So PARMS[2] is the inverse of the deviation (avoids any division by zero issues)
*/
double absoluteGaussian(double x, double q[])
...
** ITYPE = 5
**
** Exponential of the form y = q[0]^2 exp((-x * q[1]^2) + q[2]^2
**
*/
double absoluteExponential(double x, double q[])
   {
   ...
   y = q[0] * q[0] * exp(-x * q[1] * q[1]) + q[2] * q[2];
